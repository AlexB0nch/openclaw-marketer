...
async def main() -> int:
    ...
    try:
        ...
        print(f"ERROR: TELETHON_API_ID must be int, got {api_id_raw!r}.", file=sys.stderr)
        return 1
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            print(f"Already authorized as {username}")
            return 0
        ...
        me = await client.get_me()
        username = getattr(me, "username", None) or getattr(me, "first_name", "<unknown>")
        print(f"Authorized as {username}. Session saved to {session_path}")
        return 0
    finally:
